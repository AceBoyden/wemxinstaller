...
warning "You should manually run 'curl -sS https://downloads.mariadb.com/MariaDB/mariadb_repo_setup | bash' since 20.04 or lower is not supported for this script."

notice "Updating Packages..."

apt update && apt upgrade -y

warning "You should manually run 'apt-add-repository universe' since 18.04 is not supported for this script."
...
cd /var/www
rm -r wemx # This is here due to laravel complain if running again.
...
systemctl stop apache2 #I don't know but sometimes apache2 is installed for no reason.
...
do
    case $opt in
        "1")
            certbot -d $DOMAIN_URL --manual --preferred-challenges dns certonly
            break;;
        "2")
            certbot certonly --standalone -d $DOMAIN_URL
            break;;		
        "3")
            break;;
        *) echo "Invalid option $REPLY";;
...
ln -s /etc/nginx/sites-available/wemx.conf /etc/nginx/sites-enabled/wemx.conf
nginx -t
